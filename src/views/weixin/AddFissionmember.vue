  ...
  <div class="admin-userlist">
    <div style="width: 400px;padding: 10px">
      ...
      <div class="addgroup" style="padding:20px">
        ...
        <el-col :span="6">
          <el-select v-model="lbTypenum" placeholder="请选择">
            <el-option
            v-for="item in lbType"
            :key="item.liebianType"
            :label="item.desc"
            :value="item.liebianType">
              <i class="el-icon-delete" @click="clickicon(item.liebianType)"></i>&nbsp;&nbsp;<span style="width:80%">{{ item.desc }}</span>
            </el-option>
          </el-select>
        </el-col>
          <el-col :span="3">
    ...
    addmember: function(){
        if(this.wxIds == ''){
            ...
            for(var i in this.wxIds ){
                // this.numpack[i].liebianType.push(Number.parseInt(this.lbTypenum));
                // this.numpack[i].weixinId.push(this.wxIds[i]);
                this.numpack.push({
