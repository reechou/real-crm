      ...
      <el-table :data="verifySetting" style="width: 100%;margin-bottom: 80px" v-loading="loading" element-loading-text="拼命加载中">
        <el-table-column label="选择">
          <template scope="scope">
            <input type="radio" name="radio" @click="addradio(scope.row.id)">
          </template>
        </el-table-column>
        <el-table-column prop="interval" label="时间间隔"> </el-table-column>
        <el-table-column label="是否自动通过好友添加">
          <template scope='scope'>{{ifAutoVerified(scope.row.ifAutoVerified)}}</template>
        </el-table-column>
              ...
              <el-table-column property="content" label="内容">
                <template scope="scope">
                  <div v-if="scope.row.msgType == 3">
                    <img v-bind:src="scope.row.content" width="100">
                  </div>
