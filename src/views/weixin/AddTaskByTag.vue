<template>
    <div class="admin-friendslist">
        ...
        <template>
            ...
            <el-table
                :data="alltaglist" style="width: 100%;margin-bottom: 80px" v-loading="loading" element-loading-text="拼命加载中">
                <el-table-column label="id">
                    <template scope="scope">
                      <el-checkbox :label="scope.row.id"></el-checkbox>
                    </template>
                </el-table-column>                 
                <el-table-column label="用户名"><template scope='scope'>{{ scope.row.tagType }}</template></el-table-column>
                <el-table-column label="别名"><template scope='scope'>{{scope.row.groupName}}</template></el-table-column>  
            </el-table>
            </el-checkbox-group>
        </template>
    </div>
